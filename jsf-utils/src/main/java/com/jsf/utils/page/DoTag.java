...
public class DoTag {
     ...
     * @return
     */
    public static String tag(Pagination page, String queryForm) {
        // bootstrap分页样式类
        String divClass = "pagination pull-right";
        ...
        // 页数
        strBuf.append("<li><a>(第").append(page.getPageNo()).append("/").append(page.getTotalPage()).append("页").append(" 共").append(page.getTotal()).append("条)</a></li>");
        ...
        if (page.getPageNo() == 1) {
            ...
            strBuf.append(" <li><a href=\"javascript:toPage(1)\">首页</a></li> ");
            strBuf.append(" <li><a href=\"javascript:toPage(").append(page.getPageNo() - 1).append(")\"> 上一页 </a></li> ");
        }

        /*int[] nums = page.getNavigatepageNums();
        for (int i = 0; i < nums.length; i++) {
            if (page.getPageNo() == nums[i]) {
                strBuf.append("<li class=\"active\"><a>").append(nums[i]).append("</a></li>");
            } else {
                strBuf.append("<li><a href=\"javascript:toPage(").append(nums[i]).append(")\">").append(nums[i]).append("</a></li>");
            }
        ...
        if (page.getPageNo() == page.getTotalPage() || page.getTotalPage() <= 0) {
            ...
            strBuf.append(" <li><a href=\"javascript:toPage(").append(page.getPageNo() + 1).append(")\"> 下一页 </a></li> ");
            strBuf.append(" <li><a href=\"javascript:toPage(").append(page.getTotalPage()).append(")\">末页</a></li> ");
        }
        ...
        if (page.getTotalPage() > 1) {
            strBuf.append("<style>.pagination a.jump{width: 100px;}"
                    ...
                    + ".pagination button{width: 35%;height: 100%;position: absolute;right: 0px;top: 0px;padding: 0px;font-size: 12px;border: none;border-left: solid 1px #ccc;}</style>");
            strBuf.append(
                    "<li><a class=\"jump\">Jump<input type=\"text\" value=\"" + page.getPageNo() + "\" id=\"goPageNo\" class=\"form-control\" "
                            + "onkeyup=\"javascript:if(event.keyCode==13){toPage(document.getElementById('goPageNo').value)}\">"
                            + "<button class=\"form-control\" onclick=\"toPage(document.getElementById('goPageNo').value)\">GO</button></a></li>");
        }
        strBuf.append("</ul>");
